 ...
 */
function wp_rentals_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'wp_rentals' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'wp_rentals' ),
...
add_action( 'widgets_init', 'wp_rentals_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function wp_rentals_scripts() {
	wp_enqueue_style( 'wp_rentals-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'wp_rentals-style', 'rtl', 'replace' );

	wp_enqueue_script( 'wp_rentals-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'wp_rentals_scripts' );
